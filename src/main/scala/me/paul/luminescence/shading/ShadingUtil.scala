...
object ShadingUtil {
    ...
    def diffuse(weight: Double, shadedColor: ShadedColor, normal: Vector3D, toLight: Vector3D): ShadedColor = {
        shadedColor * clamp(normal.normalize dot toLight.normalize, 0.0, 1.0) * weight
    }
